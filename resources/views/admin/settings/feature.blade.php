    ...
    <div class="row">
        <div class="col-xl-3">
            <div class="page_left_bar">
                ...
                <a class="active" href="{{route('settings.feature')}}"> Features </a>
        ...
        <div class="col-xl-9 home_custome_table subcata_vt">
            <div class="card-box home_table">
                <h4 class="header-title_vt mb-3 pl-2">Feature</h4>
                    ...
                    <div class="tab-content">
                                ...
                                <table id="datatable_hybrid2"
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                {{$feature->is_active == 1 ? 'Active' : 'InActive'}}
                                            </td>
                                            <td>
                                                <div class="btn-group mb-2">
                                                    ...
                                                    <div class="dropdown-menu">
                                                        <a class="dropdown-item"
                                                            onclick="editfeature({{$feature->id}},{{json_encode($feature->feature)}},{{json_encode($feature->is_active)}})">Edit</a>
                                                        <button type="button" class="dropdown-item deleteSubCategory"
                                                            data-toggle="modal" data-target=".make-delete"
                                                            data-id="{{$feature->id}}">Delete</button>
                                                    </div>
                                                </div><!-- /btn-group -->
                                            </td>
                        ...
                        <div class="tab-pane" id="profile-b1">
                            <form class="card p-2" method="post"
                                action="{{route('feature.add', ['featureId' => $featureId])}}"
                                enctype="multipart/form-data" id="featureform">
                                {{ csrf_field() }}
                                <div class="row">
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            <label for="feature">Feature<span class="star_vt">*</span></label>
                                            <input class="form-control" type="text" id="feature" required=""
                                                placeholder="Enter Feature" name="feature" value="{{old('feature')}}">
                                        </div>
                                    </div>
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            <label for="feature-status">Status<span class="star_vt">*</span></label>
                                            <select class="form-control" id="feature-status" name="status">
...
<div class="modal fade make-delete" tabindex="-1" role="dialog" aria-labelledby="myCenterModalLabel" aria-hidden="true"
    style="display: none;">
    <div class="modal-dialog modal-dialog-centered">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <h4 class="model-heading-vt">Are you sure to delete <br>this Feature ?</h4>
                <div class="modal-footer">
                    <button type="button" class="btn_create_vt deleteFeatureConfirm">Yes, Delete</button>
...
function editfeature(id, feature, status) {
    ...
    document.getElementById('editFeature1').textContent = 'Update Feature';
    document.getElementById('submitBtn').textContent = 'Update';
    document.getElementById('editFeature2').classList.remove("active");
    document.querySelector('#feature').value = feature;
    if (status === 1) {
        status = 'Active';
    } else {
        status = 'InActive';
    }
    let child = document.getElementById('feature-status').children;
    ...
    let url = '{{ url("/settings/feature", ":featureId") }}';
    url = url.replace('%3AfeatureId', id);
    document.getElementById('featureform').setAttribute('action', url);
}
...
$('.deleteFeatureConfirm').click(function() {

    let id = $('.deleteModal #dltUserID').val();
    $.ajax({
        url: '{{route('settings.feature.delete')}}',
        type: 'post',
        data: {
            "_token": "{{csrf_token()}}",
            'id': id
        ...
        success: function(response) {
            if (response.status) {
                swal("Feature deleted successfully!");
                document.getElementById('-cancel').click();
                setTimeout(function() {
                    window.location.reload()
                }, 1000);
            } else {
                swal("Feature could not be deleted!");
                document.getElementById('-cancel').click();
                setTimeout(function() {
                    window.location.reload()
                }, 1000);
